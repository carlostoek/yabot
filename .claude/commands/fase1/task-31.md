...
Execute task 31 for the fase1 specification.

## Task Description
b. Add API server initialization to BotApplication in src/core/application.py

## Code Reuse
**Leverage existing code**: existing event bus setup, src/api/server.py
...
Execute with @spec-task-executor agent the following task: "b. Add API server initialization to BotApplication in src/core/application.py"

## Aiogram 3 Implementation Guidelines
...
### BotApplication Integration Pattern
```python
class BotApplication:
    ...
    async def _setup_api_server(self) -> None:
        ...
        # Initialize background tasks for API server
        await self.api_server.startup()

    async def start_application(self) -> None:
        """Start application with proper Aiogram 3 initialization order"""
...
async def _setup_aiogram_dispatcher(self) -> None:
    ...
    self.dp.middleware.setup(DatabaseMiddleware(self.database_manager))
...
```
Use the @spec-task-executor agent to implement task 31: "b. Add API server initialization to BotApplication in src/core/application.py" for the fase1 specification and include all the below context.
...
## Task Details
- Task ID: 31
- Description: b. Add API server initialization to BotApplication in src/core/application.py
- Leverage: existing event bus setup, src/api/server.py
- Requirements: 5.1

## Instructions
- Implement ONLY task 31: "b. Add API server initialization to BotApplication in src/core/application.py"
- Follow all project conventions and leverage existing code
- Mark the task as complete using: claude-code-spec-workflow get-tasks fase1 31 --mode complete
...
```bash
claude-code-spec-workflow get-tasks fase1 31 --mode complete
```
