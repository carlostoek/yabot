...
Execute task 30 for the fase1 specification.

## Task Description
Create database middleware in src/core/middleware.py

## Code Reuse
**Leverage existing code**: src/core/middleware.py, src/database/manager.py
...
Execute with @spec-task-executor agent the following task: "Create database middleware in src/core/middleware.py"

## Aiogram 3 Middleware Implementation Guidelines

### Required Aiogram 3 Imports
```python
...
async def start_handler(message: Message, user_service, event_bus):
    """Aiogram 3 handler with dependency injection"""
    user_id = str(message.from_user.id)

    # Access user context through injected service (Requirement 5.3.3)
    user_context = await user_service.get_user_context(user_id)

    # Publish user interaction event (Requirement 5.3.1)
    await event_bus.publish("user_interaction", {
        "user_id": user_id,
        "action": "start",
        "context": {"message_id": message.message_id}
    })

    await message.answer("Welcome to YABOT!")

# Callback query handler with database context
@main_router.callback_query(F.data.startswith("menu_"))
async def menu_callback_handler(callback: CallbackQuery, user_service):
    """Handle menu selections with database context"""
    user_id = str(callback.from_user.id)
    menu_choice = callback.data.split("_", 1)[1]

    # Update user state in MongoDB (Requirement 1.3.2)
    await user_service.update_user_state(user_id, {
        "menu_context": menu_choice,
        "last_activity": datetime.utcnow().isoformat()
    })

    await callback.answer(f"Selected: {menu_choice}")
```

### FSM Integration (if applicable)
```python
...
class NarrativeStates(StatesGroup):
    ...
    processing_story = State()

@main_router.message(StateFilter(NarrativeStates.waiting_for_choice))
async def process_narrative_choice(message: Message, state: FSMContext, user_service):
    """Process narrative choice with proper state management"""
    user_id = str(message.from_user.id)
    choice = message.text

    # Update both FSM state and database state
    await state.set_state(NarrativeStates.processing_story)
    await user_service.update_user_state(user_id, {
        "narrative_progress.last_choice": choice,
        "current_state.fsm_state": "processing_story"
    })
```

```
Use the @spec-task-executor agent to implement task 30: "Create database middleware in src/core/middleware.py" for the fase1 specification and include all the below context.
...
## Task Details
- Task ID: 30
- Description: Create database middleware in src/core/middleware.py
- Leverage: src/core/middleware.py, src/database/manager.py
- Requirements: 5.1

## Instructions
- Implement ONLY task 30: "Create database middleware in src/core/middleware.py"
- Follow all project conventions and leverage existing code
- Mark the task as complete using: claude-code-spec-workflow get-tasks fase1 30 --mode complete
...
```bash
claude-code-spec-workflow get-tasks fase1 30 --mode complete
```
